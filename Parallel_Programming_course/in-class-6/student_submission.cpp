...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < ITER_NUM; i++)
    {
        ...
        else
        {
            ...
            MPI_Recv(old_wave_map + (st_row - 1) * MAP_WIDTH, MAP_WIDTH, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            ...
            MPI_Recv(old_wave_map + (end_row % MAP_HEIGHT) * MAP_WIDTH, MAP_WIDTH, MPI_FLOAT, (rank + 1) % size, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
