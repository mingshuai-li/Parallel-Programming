...
#pragma omp parallel for schedule(static)
	...
	/* leftmost column */
	#pragma omp parallel for private(dist) schedule(static)
	for( j=1; j<np-1; j++ )
	{
...
#pragma omp parallel for private(ii, j, jj, temp, k, l) schedule(static)
    for( i=0; i<stopy; i++ ){
       ii=stepy*i;
       for( j=0; j<stopx; j++ ){
          jj=stepx*j;
